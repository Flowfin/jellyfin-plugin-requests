...
public class FulfilmentSweepTests
{
    ...
    public async Task AnObservationLosesToADecisionMadeWhileItWasBeingMade()
    {
        ...
        var sweep = new FulfilmentSweep(moving, library, new TestClock(Asked), new RecordingJournal(), new RecordingSink(), new RecordingRequesterNotice(), new RecordingLogger());
    ...
    private static FulfilmentSweep Sweep(IRequestStore store, ILibrary library, TestClock clock)
        => new FulfilmentSweep(store, library, clock, new RecordingJournal(), new RecordingSink(), new RecordingRequesterNotice(), new RecordingLogger());
