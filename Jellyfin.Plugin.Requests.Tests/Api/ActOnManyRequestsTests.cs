...
public sealed class ActOnManyRequestsTests
{
    ...
    private RequestsController ControllerFor(IRequestStore store, Guid? caller)
        => new RequestsController(store, new TestClock(Started), _identifiers, new FakeCallerIdentity(caller), new FakeInstallSettings(), new RecordingJournal(), new RecordingSink());
