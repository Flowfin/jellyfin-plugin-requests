...
| `release-install.yaml`    | adopted, landed   | The archive a published release attaches, downloaded, checked against the digest published beside it, and installed both on a server of its line and on the floor server that line's `targetAbi` names. `package.yaml` installs the package the run just built, so a publish that attached the wrong bytes passes it. A line with no release, and a line whose floor has no published image, are each reported rather than passed over. #152.                                              |
